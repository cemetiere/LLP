...
int32_t initAllocator(const char *filename) {
    ...
    if (hFile == INVALID_HANDLE_VALUE) {
        closeAllocator();
        status = INVALID_FILE_HEADER_WIN;
        return status;
    }
    uint64_t s = fileSize();
    if (s == 0) {
...
static int32_t expandFile(uint64_t size) {
    ...
    if (SetFilePointerEx(hFile, s, NULL, FILE_BEGIN) == 0) {
        status = SET_FILE_POINTER_ERROR;
        return status;
    }
    if (!SetEndOfFile(hFile)) {
        status = SET_END_OF_FILE_ERROR;
        return status;
    }
    hMap = CreateFileMapping(
            hFile,
